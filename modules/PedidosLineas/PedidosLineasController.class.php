...
class PedidosLineasController extends Controller {
    ...
    protected $parentEntity = "PedidosCab";
    ...
    public function listAction($idPedido='') {
        ...
        if ($this->values['permisos']['C']) {
            ...
            $pedido = new $this->parentEntity($idPedido);
            if ($pedido->getIDEstado()->getIDTipo() == '0') {
                // Si el pedido está pte de confirmar, puedo modificar sus líneas y
            ...
            $lis = new $this->entity();
            $rows = $lis->cargaCondicion('IDLinea', "IDPedido = '" . $idPedido . "'", 'IDLinea ASC');
            foreach ($rows as $row) {
            ...
            $this->values['linkBy']['value'] = $idPedido;
        ...
        switch ($this->request['accion']) {
            ...
            case 'B': //BORRAR DATOS
                if ($this->values['permisos']['B']) {
                    ...
                    unset($datos);
                    return $this->listAction($this->request[$this->entity]['IDPedido']);
                } else {
                    return array('template' => '_global/forbiden.html.twig');
                }
                break;
